...
GetKeyState = ctypes.windll.user32.GetKeyState # used GetAsyncKeyState before
...
def press(key_name: str) -> None:
    ...
    if key_name == 'mouse_right':
        ...
        ii_ = InputI()
        ii_.mi = MouseInput(0, 0, 0, 0x8, 0, ctypes.pointer(extra))
        command = Input(ctypes.c_ulong(0), ii_)
        ctypes.windll.user32.SendInput(1, ctypes.pointer(command), ctypes.sizeof(command))
    elif key_name == 'mouse_left':
        extra = ctypes.c_ulong(0)
        ii_ = InputI()
        ii_.mi = MouseInput(0, 0, 0, 0x2, 0, ctypes.pointer(extra))
        command = Input(ctypes.c_ulong(0), ii_)
        ctypes.windll.user32.SendInput(1, ctypes.pointer(command), ctypes.sizeof(command))
...
def release(key_name: str) -> None:
    ...
    if key_name == 'mouse_right':
        ...
        extra = ctypes.c_ulong(0)
        ii_ = InputI()
        ii_.mi = MouseInput(0, 0, 0, 0x4, 0, ctypes.pointer(extra))
        command = Input(ctypes.c_ulong(0), ii_)
        ctypes.windll.user32.SendInput(1, ctypes.pointer(command), ctypes.sizeof(command))
    elif key_name == 'mouse_middle':
        extra = ctypes.c_ulong(0)
...
def is_pressed(key_name: str) -> bool:
    ...
    :return: bool
    """
    return bool(GetKeyState(name_to_input_code[key_name]) & 2 ** 15)


def are_pressed(key_list: tuple) -> bool:
    """
    checks if all keys in key_list are being pressed
    :param key_list: tuple
    :return: bool
    """
    for key in key_list:
        if bool(GetKeyState(name_to_input_code[key]) & 2 ** 15) is False:
            return False
    return True


def is_pressed_once(key_name: str) -> bool:
    """
...
def are_pressed_once(key_list: tuple) -> bool:
    """
    checks if a set of keys is being pressed but only once :
    if the keys were being pressed during the last call and are still being pressed,
    this function returns False... until one or more key of the set is released and pressed again
    :param key_list: str
    :return: bool
    """
    if are_pressed(key_list):
        if key_list in is_on and is_on[key_list]:
            return False
        else:
            is_on[key_list] = True
            return True
    elif (key_list in is_on and is_on[key_list]) or (key_list not in is_on):
        is_on[key_list] = False
        return False
...
def move_mouse_absolute(x, y) -> None:
    """
    moves mouse relatively
    :param x: float
    :param y : float
...
def continuous_relative_move(x: int, y: int, time_interval: int) -> None:
    """
